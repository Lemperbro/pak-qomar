            ...
            <form action="{{ route('penjualan.update', ['id' => $data->id]) }}" method="POST" class="max-w-[800px] mx-auto">
                ...
                <div>
                    ...
                    @enderror">
                        ...
                        <option value="{{ $desa[$i] }}" class="uppercase" {{ $desa[$i] == $data->desa ? 'selected' : '' }}>
                            {{ $desa[$i] }}
                        </option>
                    @endfor
                    </select>
                ...
                <div>
                    ...
                    @enderror">
                        ...
                        <option value="{{ $kecamatan[$i] }}" class="uppercase" {{ $kecamatan[$i] == $data->kecamatan ? 'selected' : '' }}>
                            {{ $kecamatan[$i] }}
                        </option>
                    @endfor
                    </select>
                ...
                <div class="mt-3">
                    ...
                    <input type="number" name="tps" id="tps"
                        class="border-[1px] border-main3 rounded-md w-full focus:ring-0 focus:outline-none focus:border-main2 mt-1 @error('tps')
                            peer
                        @enderror"
                        placeholder="Contoh: 1" value="{{ $data->tps }}">
                    @error('tps')
                        <p class="peer-invalid:visible text-red-700 font-light">
                            {{ $message }}
                        </p>
                ...
                <div class="mt-3">
                    ...
                    <input type="number" name="total_suara" id="total_suara"
                        class="border-[1px] border-main3 rounded-md w-full focus:ring-0 focus:outline-none focus:border-main2 mt-1 @error('total_suara')
                            peer
                        @enderror"
                         placeholder="Contoh: 10000" value="{{ $data->jumlah_pemilih }}">
                    @error('total_suara')
                        <p class="peer-invalid:visible text-red-700 font-light">
                            {{ $message }}
                        </p>
